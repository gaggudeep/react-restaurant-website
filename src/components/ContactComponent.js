...
import { Breadcrumb, BreadcrumbItem, Button, Form, FormGroup, Label, Input, Col, Row, FormFeedback, InputGroup } from 'reactstrap';
...
class Contact extends Component {
    ...
    validate(firstname, lastname, telnum, email) {
        ...
        if(this.state.touched.firstname && firstname.length < 3)
            errors.firstname = "First name should be greater than equal to 3 characters";
        else if(this.state.touched.firstname && firstname.length > 20)
            errors.firstname = "First name should be smaller than equal to 20 characters";

        if(this.state.touched.lastname && lastname.length < 3)
            errors.lastname = "Last name should be greater than equal to 3 characters";
        else if(this.state.touched.lastname && lastname.length > 20)
            errors.lastname = "Last name should be smaller than equal to 20 characters";
        ...
        if(this.state.touched.email && email.split('').filter(x => x === '@').length !== 1)
            errors.email = "Email should contain a single @";
                        ...
                        <Form onSubmit={this.handleSubmit}>
                            ...
                            <FormGroup row>
                                ...
                                <Col md={{size: 3, offset: 1}}>
                                    <Input type="select" name="contactType" value={this.state.contacType} 
                                    onChange={this.handleInputChange}>
                                        <option>Tel.</option>
                                        <option>Email</option>
                                    </Input>
                            ...
                            <FormGroup row>
                                <Label htmlFor="feedback" md={2}>Your Feeback</Label>
                                <Col  md={10}>
